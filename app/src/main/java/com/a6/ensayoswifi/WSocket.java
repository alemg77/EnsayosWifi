...
public class WSocket {
    ...
    private final static String TAG = WSocket.class.getSimpleName();
    ...
    public static class Builder {
        ...
        public WSocket build() {
            return new WSocket(request.build());
        }
    }
    ...
    private WSocket(Request request) {
     ...
     */
    public WSocket onEvent(@NonNull String event, @NonNull OnEventListener listener){
     ...
     * @param listener message on arrive listener
     */
    public WSocket onEventResponse(@NonNull String event, @NonNull OnEventResponseListener listener){
    ...
    /**
     * Set state listener which fired every time {@link WSocket#state} changed.
     *
     * @param listener state change listener
     */
    public WSocket setOnChangeStateListener(@NonNull OnStateChangeListener listener) {
     ...
     */
    public WSocket setMessageListener(@NonNull OnMessageListener listener) {
     ...
     * {@link OnStateChangeListener#onChange(WSocket, State)}
     * @param newState new state
     */
    private void changeState(State newState) {
        state = newState;
        if (onChangeStateListener != null) {
            onChangeStateListener.onChange(WSocket.this, state);
        }
    }
    ...
    private void reconnect() {
        ...
        if (eventListener.get(EVENT_RECONNECT_ATTEMPT) != null) {
            eventListener.get(EVENT_RECONNECT_ATTEMPT).onMessage(WSocket.this, EVENT_RECONNECT_ATTEMPT);
        }
    ...
    private WebSocketListener webSocketListener = new WebSocketListener() {
        ...
        public void onOpen(WebSocket webSocket, Response response) {
            ...
            if (eventListener.get(EVENT_OPEN) != null) {
                eventListener.get(EVENT_OPEN).onMessage(WSocket.this, EVENT_OPEN);
            }
        ...
        public void onMessage(WebSocket webSocket, String text) {
            ...
            if (messageListener != null)
                messageListener.onMessage(WSocket.this, text);
            ...
            try {
                ...
                if (eventResponseListener.get(event) != null) {
                    eventResponseListener.get(event).onMessage(WSocket.this, event, data);
                }
                // call event listener
                if (eventListener.get(event) != null) {
                    eventListener.get(event).onMessage(WSocket.this, event);
                }
            } catch (JSONException e) {
        ...
        public void onClosed(WebSocket webSocket, int code, String reason) {
            ...
            if (eventListener.get(EVENT_CLOSED) != null) {
                eventListener.get(EVENT_CLOSED).onMessage(WSocket.this, EVENT_CLOSED);
            }
        }
    ...
    public abstract static class OnMessageListener {
         ...
         */
        private void onMessage (WSocket socket, final String data) {
    ...
    public abstract static class OnEventListener {
        ...
        private void onMessage (WSocket socket, final String event) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    onMessage(event);
    ...
    public abstract static class OnEventResponseListener extends OnEventListener {
         ...
         */
        private void onMessage (WSocket socket, final String event, final JSONObject data) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
    ...
    public abstract static class OnStateChangeListener {
         ...
         */
        private void onChange (WSocket socket, final State status){
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    onChange(status);
